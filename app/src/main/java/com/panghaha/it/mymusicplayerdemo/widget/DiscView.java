...
import com.panghaha.it.mymusicplayerdemo.adapter.CommonViewPagerAdapter;
import com.panghaha.it.mymusicplayerdemo.adapter.ViewPagerHolder;
import com.panghaha.it.mymusicplayerdemo.adapter.ViewPagerHolderCreator;
...
public class DiscView extends RelativeLayout {
    ...
    private ObjectAnimator CoverAnimator;//公用动画
    ...
    private void initViewPager() {
        commonViewPagerAdapter = new CommonViewPagerAdapter(new ViewPagerHolderCreator() {
            @Override
        ...
        mVpContain.setAdapter(commonViewPagerAdapter);
...
//            CoverAnimator.cancel();
    ...
    public void setMusicDataList(List<Song> musicDataList, int currentPosition) {
...
//        int i = 0;
//        for (Song musicData : mMusicDatas) {
...
//            disc.setImageDrawable(getDiscDrawable(musicData.image));
//
//            mDiscAnimators.add(getDiscObjectAnimator(disc, i++));
//            mDiscLayouts.add(discLayout);
//        }
        commonViewPagerAdapter.setDataList(mMusicDatas);

        Song musicData = mMusicDatas.get(currentPosition);
        if (mIPlayInfo != null) {
            mIPlayInfo.onMusicInfoChanged(musicData.song, musicData.singer);
            mIPlayInfo.onMusicPicChanged(musicData.image);
        }

        mVpContain.setCurrentItem(currentPosition);
    }

    private void setCoverAnimator(View disc, final int i) {
        CoverAnimator = ObjectAnimator.ofFloat(disc, View.ROTATION, 0, 360);
...
//        CoverAnimator.pause();
    ...
    public class PageHolder implements ViewPagerHolder {

        private View discLayout;
        private ImageView disc;

        @Override
        public View createView(Context context) {
            discLayout = LayoutInflater.from(getContext()).inflate(R.layout.layout_disc, mVpContain, false);
            disc = (ImageView) discLayout.findViewById(R.id.ivDisc);
            return discLayout;
        }

        @Override
        public void onBind(Context context, int position, Song data) {
            disc.setImageDrawable(getDiscDrawable(data.image));
//            mDiscAnimators.add(getDiscObjectAnimator(disc, position));
            setCoverAnimator(disc,position);
            mDiscLayouts.add(discLayout);
        }
    }

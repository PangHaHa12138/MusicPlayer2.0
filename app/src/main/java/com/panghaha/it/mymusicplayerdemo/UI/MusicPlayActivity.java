...
public class MusicPlayActivity extends AppCompatActivity implements IPlayInfo, View.OnClickListener {
    ...
    private void initMusicDatas(Intent intent) {
        mMusicDatas = (List<Song>) intent.getSerializableExtra(MusicPlayActivity.PARAM_MUSIC_LIST);
        String song = intent.getStringExtra("song");
        String singer = intent.getStringExtra("singer");
        int image = intent.getIntExtra("image", 0);
        int position = intent.getIntExtra("position", 0);
        mDisc.setMusicDataList(mMusicDatas,position);
        play(position-1);
    ...
    private void play(int position) {
        if (position != -1) {
            ...
            intent.setAction(MusicService.ACTION_OPT_MUSIC_PLAY);

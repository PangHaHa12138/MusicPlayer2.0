...
public class MusicService extends Service implements MediaPlayer.OnCompletionListener {
    ...
    private void initMusicDatas(Intent intent) {
        ...
        List<Song> musicDatas =
                (List<Song>) intent.getSerializableExtra(MusicPlayActivity.PARAM_MUSIC_LIST);
        mMusicDatas.clear();
    ...
    private void play(final int index) {
        ...
        if (mCurrentMusicIndex == index && mIsMusicPause) {
            ...
            mMediaPlayer.stop();
            mMediaPlayer = null;
            if (!TextUtils.isEmpty(mMusicDatas.get(index).path)) {
                Uri path = Uri.parse(mMusicDatas.get(index).path);
                mMediaPlayer = MediaPlayer.create(getApplicationContext(), path);
            } else {
                mMediaPlayer = MediaPlayer.create(getApplicationContext(), mMusicDatas.get(index).path_local);
            }
